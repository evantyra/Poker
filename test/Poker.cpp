...
void Poker::drawHand(Hand *hand, bool hidden, Frame *frame) {
    ...
    if (hidden) { //top player
        for (int i = 0; i < 5; i++) {
            GAME_DISPLAY.drawBox(x + 1 + i*7, y + 2, 6, 4, 0);
        }
...
void Poker::updateGameInfo() {
    ...
    bannerText << bannerHeader << " /My Money: " << m_HumanPlayer.getMoney() << "/Money In Pot: " << m_PotMoney;
...
void Poker::runBetting(int bettingCycle) {
    ...
    while (true) {
        ...
        if (callCounter == m_Players.size() || allPlayersAllIn())
            break;
